...
import { ArrowDownToLine, ArrowRight, Check, ChevronLeft, ChevronRight, CircleCheck, FileCheck2, FileText, FolderOpen, Grip, Image as ImageIcon, LoaderCircle, LockKeyhole, Minus, MousePointer2, PenLine, Plus, ShieldCheck, Trash2, Upload, X } from "lucide-react";
    ...
    try {
      ...
      if (detected.kind === "pdf") {
        ...
        try {
          ...
          docRef.current = newDoc; imageRef.current = null; bytesRef.current = bytes;
          setImage(null); setPdf(newDoc); setFilename(file.name); setFilesize(file.size);
          ...
          const isPasswordError = e instanceof Error && (e.name === "PasswordException" || /password|encrypt/i.test(message));
          setError(isPasswordError
            ? "Ce PDF est protégé par un mot de passe. Ouvrez une copie déverrouillée pour la signer."
            : "Impossible d’ouvrir ce fichier. Vérifiez qu’il s’agit d’un PDF valide et non corrompu.");
        }
      } else {
        try {
          const bytes = new Uint8Array(await file.arrayBuffer());
